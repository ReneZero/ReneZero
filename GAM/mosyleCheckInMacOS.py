    ...
    for device in json_data['response']['devices']:
        if 'userid' in device:
            if device['userid'] == userID:
                ...
                while device['userid'] != userID:
                    for test in editPayload['options']['page']:
                        test=str(counter)
                pageUp = json.dumps(editPayload)
                r3 = requests.post('https://managerapi.mosyle.com/v2/listdevices', data=pageUp)
